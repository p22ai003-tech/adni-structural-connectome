#!/usr/bin/env python3
"""Execute the approved retry4 15-unit pre-tractography recovery only.

The launcher requires both the immutable SL-H04A-CAL release and the exact
SL-H04A-CAL-R1 postcondition hotfix. It reuses valid completed work, reruns
only the unresolved bounded DAG, and stops after automated QC plus blinded
review bundles. No tractography, SIFT2, matrix, statistics, dashboard,
preprocessing/Eddy, or response re-estimation rule is reachable.
"""
...
HOTFIX_BINDING_PATH = PACKAGE / "hotfix_execution_binding.json"
APPROVAL_RELEASE = PACKAGE / "approval_release_validation.json"
PACKAGE_VALIDATION = PACKAGE / "validation.json"
PRIOR_COMPLETION_PATH = RUN_ROOT / "publication/pre_tractography_canary_completion.json"
EARLY_STOP_DECISION_PATH = (
    AUDIT / "decisions/sl_p0_16b_known_defect_early_stop_20260719.json"
)
COMPLETION_PATH = RUN_ROOT / "publication/pre_tractography_canary_recovery1_completion.json"
RUN_CONTEXT_PATH = RUN_ROOT / "contract/pre_tractography_canary_recovery1_run_context.json"
...
from finalize_h04a_r1_retry4_pretract_recovery1 import (  # noqa: E402
    LEDGER_NAME,
    MANIFEST_NAME,
    finalize_recovery1,
)
...
def validate_hotfix_binding(
    ...
    if (
        ...
        raise PermissionError("pretract recovery hotfix binding differs")
    decision_path = verify_record(
        binding.get("hotfix_decision"), label="hotfix decision"
    )
    decision = load_json(decision_path)
    if (
        decision.get("status") != "APPROVED"
        or decision.get("decision_gate") != "SL-H04A-CAL-R1"
        or decision.get("required_user_response") != "Approve SL-H04A-CAL-R1"
        or decision.get("user_response") != "Approve SL-H04A-CAL-R1"
        or decision.get("run_root") != str(RUN_ROOT)
        or decision.get("units") != execution_binding.get("units")
    ):
        raise PermissionError("exact hotfix decision approval differs")
    reuse_record = decision.get("reusable_output_inventory")
    if not isinstance(reuse_record, Mapping):
        raise TypeError("hotfix decision lacks reusable output inventory record")
    reuse_path = verify_record(reuse_record, label="reusable output inventory")
    reuse = load_json(reuse_path)
    files = reuse.get("files")
    if (
        reuse.get("schema_version") != "1.0.0"
        or reuse.get("record_type")
        != "retry4_pretract_recovery_reuse_inventory"
        or reuse.get("status") != "PASS"
        or reuse.get("run_root") != str(RUN_ROOT)
        or not isinstance(files, list)
        or reuse.get("file_count") != len(files)
        or reuse.get("total_bytes")
        != sum(
        ...
        raise ValueError("reusable output inventory contract differs")
    for index, record in enumerate(files):
        if not isinstance(record, Mapping):
            raise TypeError(f"reusable output record {index} is not a mapping")
        path = verify_record(record, label=f"reusable output {index}")
        if path.is_symlink():
            raise ValueError(f"reusable output became a symlink: {path}")
        try:
            relative = path.relative_to(RUN_ROOT).as_posix()
        except ValueError as exc:
            raise ValueError(f"reusable output escaped run root: {path}") from exc
        if record.get("relative_path") != relative:
            raise ValueError(f"reusable output relative path differs: {path}")
    if binding.get("prior_failed_completion") != base.file_record(
        PRIOR_COMPLETION_PATH
    ):
        raise ValueError("hotfix no longer binds the immutable failed completion")
    if binding.get("prior_early_stop_decision") != base.file_record(
        EARLY_STOP_DECISION_PATH
    ):
        raise ValueError("hotfix no longer binds the controlled early-stop decision")
    if binding.get("prior_pretract_extension_binding") != base.file_record(
        PRETRACT_BINDING_PATH
    ):
        raise ValueError("hotfix prior pretract authority differs")
    if binding.get("response_calibration_binding") != base.file_record(
        RESPONSE_BINDING_PATH
    ):
        raise ValueError("hotfix response authority differs")
    for label, record in binding.get("source_files", {}).items():
        verify_record(record, label=f"hotfix source {label}")
...
def pretract_resource_preflight(
    *, decision_sha256: str, requested_cores: int
    ...
    for start_path in sorted(attempts_dir.glob("*.start.json")):
        ...
        if start.get("launcher_mode") not in {
            "pre-tractography-canary",
            "pre-tractography-canary-recovery1",
        }:
            continue
...
def main() -> int:
    ...
    hotfix_binding = load_json(HOTFIX_BINDING_PATH)
    prior_completion = load_json(PRIOR_COMPLETION_PATH)
    recovery_binding = load_json(RECOVERY_BINDING)
    if (
        package.get("status") != "PASS"
        or int(package.get("scheduled_job_counts", {}).get("total", 0)) < 1
        or package.get("forbidden_scheduled_rules") != []
        or release.get("status") != "PASS"
        or release.get("imaging_executed") is not False
        or release.get("hotfix_execution_binding")
        != base.file_record(HOTFIX_BINDING_PATH)
        or prior_completion.get("record_type")
        != "pre_tractography_canary_completion"
        ...
        raise PermissionError("SL-H04A-CAL-R1 recovery release binding differs")
    validate_pretract_extension_binding(
        pretract_binding,
    ...
    hotfix_decision_path = validate_hotfix_binding(
        hotfix_binding, execution_binding=execution_binding
    )
    ...
    hotfix_decision_sha256 = base.sha256_file(hotfix_decision_path)
    resource = pretract_resource_preflight(
        decision_sha256=hotfix_decision_sha256,
    ...
    identity = {
        ...
        "prior_failure_completion_sha256": base.sha256_file(PRIOR_COMPLETION_PATH),
        "hotfix_decision_sha256": hotfix_decision_sha256,
    }
    ...
    run_context = {
        ...
        "pretract_recovery_hotfix_binding": copy.deepcopy(hotfix_binding),
        "prior_failed_completion": base.file_record(PRIOR_COMPLETION_PATH),
        "prior_early_stop_decision": base.file_record(EARLY_STOP_DECISION_PATH),
    ...
    attempt_id = f"{timestamp}-pre-tractography-recovery1-{os.urandom(4).hex()}"
    ...
    attempt_start = {
        ...
        "lineage_id": lineage_id,
        "launcher_mode": "pre-tractography-canary",
        "recovery_mode": "pre-tractography-canary-recovery1",
        "recipe_id": completion["recipe_id"],
        ...
        "pretract_extension_binding": copy.deepcopy(pretract_binding),
        "pretract_recovery_hotfix_binding": copy.deepcopy(hotfix_binding),
        "pretract_decision": base.file_record(hotfix_decision_path),
        "prior_failed_completion": base.file_record(PRIOR_COMPLETION_PATH),
        "prior_early_stop_decision": base.file_record(EARLY_STOP_DECISION_PATH),
    ...
    completion_record = {
        ...
        "record_type": "pre_tractography_canary_recovery_completion",
