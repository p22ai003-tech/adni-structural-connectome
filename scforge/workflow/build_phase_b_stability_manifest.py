#!/usr/bin/env python3
"""Assemble immutable phase-B stability run records into one blind manifest."""
...
RUN_IDS = {"primary_3m", "primary_5m", "primary_10m", "independent_10m"}
MATRIX_NAMES = {"count", "fd_sum", "fa_mean", "md_mean", "rd_mean", "ad_mean"}
ARTIFACT_NAMES = {"tractogram", "sift2_weights", "assignments", "tractography_metadata"}
FORBIDDEN_KEYS = {
    "diagnosis",
    "diagnosis_at_dti",
    "group",
...
def build_manifest(run_record_paths: list[Path]) -> dict[str, Any]:
    ...
    for source_path in run_record_paths:
        ...
        if (
            raw.get("record_type") != "diagnosis_blind_phase_b_stability_run"
            or raw.get("diagnosis_labels_used") is not False
        ):
            raise ValueError(f"run record is not a diagnosis-blind stability record: {source_path}")
        ...
        if not (
            min(streamline_count, tractogram_count, weight_count, assignment_count) > 0
            and
            streamline_count == tractogram_count == weight_count == assignment_count
            and 0 <= assigned_count <= assignment_count
        ):
            raise ValueError(f"tractogram/weight/assignment counts differ for {unit}/{run_id}")
        calculated_fraction = assigned_count / assignment_count
        if abs(assignment_fraction - calculated_fraction) > 1.0e-12:
            raise ValueError(f"assignment fraction/counts differ for {unit}/{run_id}")
        seed_id = str(raw.get("seed_id", "")).strip()
        seed_class = str(raw.get("seed_class", "")).strip()
        if not seed_id or seed_class not in {"primary", "independent"}:
    ...
    if incomplete:
        raise ValueError(f"incomplete four-run unit sets: {incomplete}")
    return {
