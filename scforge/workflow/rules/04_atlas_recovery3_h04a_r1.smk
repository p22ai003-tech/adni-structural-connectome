"""Recovery3 single-resampling AAL3v1-to-DWI route and label contract."""
...
rule aal3_to_dwi_single_resample:
    ...
    output:
        subject_path(
            "{unit}", "04_atlas", "aal3_nodes_166_dwi_1mm_recovery3.nii.gz"
        ),
    log:
        subject_log("{unit}", "04_ants_compose_atlas_recovery3.log"),
    threads: 8
    shell:
        r"""
        set -euo pipefail
...
rule atlas_contract_qc:
    ...
    output:
        subject_path("{unit}", "04_atlas", "atlas_contract_qc_recovery3.json"),
    log:
        subject_log("{unit}", "04_atlas_contract_qc_recovery3.log"),
    run:
        ...
        atlas_path = str(input.atlas)
        reference_path = str(input.reference)
        ...
        record = {
            ...
            "node_table_sha256": _sha256(str(input.node_table)),
            "node_map_sha256": _sha256(str(input.node_map)),
            "failures": failures,
        }
...
rule atlas_native_grid_qc_copy:
    ...
    output:
        subject_path(
            "{unit}",
            "04_atlas",
            "aal3_nodes_166_native_b0_qc_only_recovery3.nii.gz",
        ),
    log:
        subject_log("{unit}", "04_atlas_native_grid_qc_copy_recovery3.log"),
    shell:
        r"""
        set -euo pipefail
        mkdir -p "$(dirname {output:q})" "$(dirname {log:q})"
