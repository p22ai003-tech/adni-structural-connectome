"""Recovery3 5TT resampling, frozen-response SS3T-CSD, and tensor maps.

The DWI-space 5TT is linearly interpolated, then clipped only to the physical
partial-volume range [0, 1].  This preserves partial-volume information while
removing floating-point overshoots before the mandatory ``5ttcheck`` gate.
"""
...
rule five_tt_dwi:
    ...
    output:
        five_tt=subject_path("{unit}", "05_model", "5tt_dwi_recovery3.mif"),
        check=subject_path("{unit}", "05_model", "5tt_dwi_recovery3_check.done"),
    log:
        subject_log("{unit}", "05_5tt_to_dwi_recovery3.log"),
    threads: 4
    shell:
        r"""
        set -euo pipefail
        mkdir -p "$(dirname {output.five_tt:q})" "$(dirname {log:q})"
        export PATH={TOOL_PATH:q}
        raw="$(dirname {output.five_tt:q})/5tt_dwi_recovery3_unclipped.partial.mif"
        trap 'rm -f "$raw"' EXIT
...
rule gmwmi_dwi:
    ...
    output:
        subject_path("{unit}", "05_model", "gmwmi_dwi_recovery3.mif"),
    log:
        subject_log("{unit}", "05_5tt2gmwmi_recovery3.log"),
    shell:
        r"""
        set -euo pipefail
        mkdir -p "$(dirname {output:q})" "$(dirname {log:q})"
