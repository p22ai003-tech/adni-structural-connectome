"""Recovery3 quantitative spatial gates and blinded visual-QC hand-off."""
...
rule spatial_qc_images:
    ...
    output:
        five_tt_mask=subject_path(
            "{unit}", "06_preflight", "5tt_mask_recovery3.nii.gz"
        ),
        dwi_mask=subject_path(
            "{unit}", "06_preflight", "dwi_mask_recovery3.nii.gz"
        ),
        dwi_mask_one_mm=subject_path(
            "{unit}", "06_preflight", "dwi_mask_1mm_recovery3.nii.gz"
        ),
    log:
        subject_log("{unit}", "06_spatial_qc_images_recovery3.log"),
    threads: 4
    shell:
        r"""
        set -euo pipefail
...
rule spatial_quantitative_qc:
    ...
    output:
        subject_path(
            "{unit}", "06_preflight", "spatial_quantitative_qc_recovery3.json"
        ),
    log:
        subject_log("{unit}", "06_spatial_quantitative_qc_recovery3.log"),
    run:
        ...
        failures = []
        five_tt = np.asanyarray(nib.load(str(input.five_tt_mask)).dataobj) > 0
        dwi = np.asanyarray(nib.load(str(input.dwi_mask)).dataobj) > 0
        ...
        atlas = np.asanyarray(nib.load(str(input.atlas)).dataobj) > 0
        dwi_one_mm = np.asanyarray(nib.load(str(input.dwi_mask_one_mm)).dataobj) > 0
...
rule visual_review_bundle:
    ...
    output:
        b0_t1=subject_path(
            "{unit}", "06_preflight", "review_b0_vs_t1_recovery3.png"
        ),
        b0_5tt=subject_path(
            "{unit}", "06_preflight", "review_b0_vs_5tt_recovery3.png"
        ),
        b0_atlas=subject_path(
            "{unit}", "06_preflight", "review_b0_vs_aal3_recovery3.png"
        ),
        index=subject_path(
            "{unit}", "06_preflight", "visual_review_index_recovery3.csv"
        ),
    log:
        subject_log("{unit}", "06_visual_review_bundle_recovery3.log"),
    shell:
        r"""
        set -euo pipefail
...
rule automated_pre_tractography_qc:
    ...
    output:
        report=subject_path(
            "{unit}", "06_preflight", "automated_pre_tractography_qc_recovery3.json"
        ),
    log:
        subject_log("{unit}", "06_automated_pre_tractography_qc_recovery3.log"),
    run:
        import subprocess
        ...
        Path(output[0]).parent.mkdir(parents=True, exist_ok=True)
        Path(log[0]).parent.mkdir(parents=True, exist_ok=True)
        wmfod_l0_path = Path(output.report).with_name("wmfod_l0_qc_recovery3.mif")
