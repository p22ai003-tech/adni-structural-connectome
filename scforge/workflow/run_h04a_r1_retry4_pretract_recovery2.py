#!/usr/bin/env python3
"""Execute the approved 15-unit retry4 pre-tractography Recovery2 only."""
...
RECOVERY2_BINDING_PATH = PACKAGE / "recovery2_execution_binding.json"
...
PRIOR_COMPLETION_PATH = RUN_ROOT / "publication/pre_tractography_canary_recovery1_completion.json"
COMPLETION_PATH = RUN_ROOT / "publication/pre_tractography_canary_recovery2_completion.json"
RUN_CONTEXT_PATH = RUN_ROOT / "contract/pre_tractography_canary_recovery2_run_context.json"
...
from finalize_h04a_r1_retry4_pretract_recovery2 import (  # noqa: E402
    LEDGER_NAME,
    MANIFEST_NAME,
    finalize_recovery2,
)
...
def validate_recovery2_binding(
    binding: Mapping[str, Any], *, execution_binding: Mapping[str, Any]
) -> tuple[Path, Path]:
    if (
        binding.get("schema_version") != "1.0.0"
        or binding.get("record_type") != "retry4_pretract_recovery2_execution_binding"
        ...
        raise PermissionError("Recovery2 execution binding differs")
    decision_path = verify_record(binding["recovery2_decision"], label="Recovery2 decision")
    decision = load_json(decision_path)
    if (
        decision.get("status") != "APPROVED"
        or decision.get("decision_gate") != "SL-H04A-CAL-R2"
        or decision.get("required_user_response") != "Approve SL-H04A-CAL-R2"
        or decision.get("user_response") != "Approve SL-H04A-CAL-R2"
        or decision.get("run_root") != str(RUN_ROOT)
        or decision.get("units") != execution_binding.get("units")
    ):
        raise PermissionError("Recovery2 exact approval differs")
    if binding.get("prior_recovery1_completion") != base.file_record(PRIOR_COMPLETION_PATH):
        raise ValueError("Recovery2 no longer binds the immutable Recovery1 completion")
    ...
    if converter_path != CONVERTER_CONTRACT.resolve():
        raise ValueError("Recovery2 converter contract path differs")
    ...
    if (
        ...
        raise ValueError("Recovery2 converter binary differs")
    ...
    if (
        reuse.get("record_type") != "retry4_pretract_recovery2_reuse_inventory"
        or reuse.get("status") != "PASS"
        or not isinstance(files, list)
        or reuse.get("file_count") != len(files)
    ):
        raise ValueError("Recovery2 reuse inventory differs")
    ...
    for label, record in binding.get("source_files", {}).items():
        verify_record(record, label=f"Recovery2 source {label}")
...
def main() -> int:
    ...
    if COMPLETION_PATH.exists() or RUN_CONTEXT_PATH.exists():
        raise FileExistsError("Recovery2 completion or run context already exists")
    for path in (
        CONFIG,
        ENVIRONMENT,
        SNAKEFILE,
    ...
    recovery2_binding = load_json(RECOVERY2_BINDING_PATH)
    if (
        ...
        or release.get("recovery2_execution_binding") != base.file_record(RECOVERY2_BINDING_PATH)
        ...
        raise PermissionError("Recovery2 release binding differs")
    validate_pretract_extension_binding(
        pretract_binding,
        expected_run_root=RUN_ROOT,
        execution_binding=execution_binding,
    )
    decision_path, dag_config = validate_recovery2_binding(
        recovery2_binding, execution_binding=execution_binding
    )
    ...
    identity = {
        ...
        "mode": "pre-tractography-canary-recovery2",
        "prior_recovery1_completion_sha256": base.sha256_file(PRIOR_COMPLETION_PATH),
        "recovery2_decision_sha256": decision_sha256,
    ...
    run_context = {
        ...
        "recovery_mode": "pre-tractography-canary-recovery2",
        ...
        "recovery2_execution_binding": copy.deepcopy(recovery2_binding),
        "prior_recovery1_completion": base.file_record(PRIOR_COMPLETION_PATH),
    ...
    attempt_id = f"{timestamp}-pre-tractography-recovery2-{os.urandom(4).hex()}"
    ...
    attempt_start = {
        ...
        "pretract_extension_binding": copy.deepcopy(pretract_binding),
        "recovery2_execution_binding": copy.deepcopy(recovery2_binding),
        "recovery2_decision": base.file_record(decision_path),
        "prior_recovery1_completion": base.file_record(PRIOR_COMPLETION_PATH),
    ...
    if (
        ...
        or dry_output.count(str(yaml.safe_load(CONVERTER_CONTRACT.read_text())["binary"]["path"])) != 15
    ):
        raise RuntimeError(
            f"Recovery2 live dry-run differs: returncode={dry.returncode}, counts={counts}, forbidden={forbidden}, unexpected={unexpected}"
        )
    ...
    completion_record = {
        ...
        "record_type": "pre_tractography_canary_recovery2_completion",
