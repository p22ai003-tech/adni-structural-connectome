#!/usr/bin/env python3
"""Fail-closed launcher for the approved SL-H04A-R1 Phase-A recovery.
...
RUN_MODES = ("response-calibration-phase-a",)
CANARY_MIN_UNITS = 12
CANARY_MAX_UNITS = 24
H04A_AUTHORIZED_MODES = ("response-calibration-phase-a",)
H04A_AUTHORIZED_THROUGH = "response_calibration_phase_a_freeze_decision"
H04A_MAXIMUM_CORES = 32
...
def prepare_h04a_resource_preflight(
    *, run_root: Path, execution_binding: dict[str, Any]
    ...
    decision_path = Path(
        str(execution_binding["execution_subset_decision"]["path"])
    ).expanduser().resolve()
    decision = _load_json_mapping(
        decision_path, label="H04A execution-subset decision"
    )
    retry_record = decision.get("execution_retry")
    externally_charged_duration = 0.0
    if isinstance(retry_record, dict):
        charged = retry_record.get("prior_duration_seconds_charged", 0.0)
        if isinstance(charged, bool) or not isinstance(charged, (int, float)):
            raise ValueError("execution retry prior-duration charge is invalid")
        externally_charged_duration = float(charged)
        if (
            not math.isfinite(externally_charged_duration)
            or externally_charged_duration < 0
        ):
            raise ValueError("execution retry prior-duration charge is invalid")
        if externally_charged_duration:
            prior_attempts.insert(
                0,
                {
                    "source": "immutable_execution_retry_evidence",
                    "duration_seconds": externally_charged_duration,
                    "execution_decision": file_record(decision_path),
                },
            )
            prior_duration += externally_charged_duration
...
def validate_existing_run_context(
    ...
    *,
    ...
    human_qc: Path | None,
    execution_binding: dict[str, Any],
    recovery_extension_binding: dict[str, Any],
    ...
    if record.get("recovery_extension_binding") != recovery_extension_binding:
        raise ValueError("existing run context R1 extension binding differs")
    if human_qc is None:
...
def run(args: argparse.Namespace) -> int:
    ...
    recovery_extension_path = args.recovery_extension_binding.expanduser().resolve()
    recovery_extension_binding = validate_recovery_extension_binding(
        _load_json_mapping(
            recovery_extension_path, label="R1 recovery extension binding"
        ),
    ...
    if (
        recovery_extension_binding["execution_manifest"]
        != execution_binding["execution_subset_manifest"]
        or recovery_extension_binding["execution_decision"]
        != execution_binding["execution_subset_decision"]
    ):
        raise ValueError("R1 extension and execution-subset evidence differ")
    ...
    if run_context_path.exists():
        ...
        run_context = {
            ...
            "execution_binding": copy.deepcopy(execution_binding),
            "recovery_extension_binding": copy.deepcopy(
                recovery_extension_binding
            ),
            "normative_config": file_record(NORMATIVE_CONFIG),
            "environment_contract": file_record(ENVIRONMENT_CONTRACT),
    ...
    resolved_config["recovery_extension_binding"] = copy.deepcopy(
        recovery_extension_binding
    )
...
def main() -> int:
    ...
    parser.add_argument("--recovery-extension-binding", type=Path, required=True)
    parser.add_argument("--cores", type=int, default=1)
    parser.add_argument(
        "--mode", choices=RUN_MODES, default="response-calibration-phase-a"
    )
    parser.add_argument(
