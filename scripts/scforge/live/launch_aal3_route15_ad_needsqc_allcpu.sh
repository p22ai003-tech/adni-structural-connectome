...
ROUTE15_WORKERS="${ROUTE15_WORKERS:-14}"
ROUTE15_THREADS="${ROUTE15_THREADS:-4}"
ROUTE15_SELECT_STREAMLINES="${ROUTE15_SELECT_STREAMLINES:-12000000}"
ROUTE15_LIMIT="${ROUTE15_LIMIT:-0}"
...
if [[ "${#SUBJECTS[@]}" -eq 0 ]]; then
  echo "no AD needs-QC subjects without R15 remaining"
  exit 0
fi
...
echo "Route15 AD needs-QC all-CPU launch | $(date -u +%Y-%m-%dT%H:%M:%SZ)"
