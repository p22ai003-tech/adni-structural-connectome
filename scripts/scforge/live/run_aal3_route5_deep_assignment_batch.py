#!/usr/bin/env python3
"""Run route-5 deep endpoint-assignment probes for persistent AAL3 failures."""
...
def queue_subjects(route3_root: Path, route4_root: Path) -> list[str]:
    sids: list[str] = []
    for row in read_csv(route3_root / "route_qc_decisions.csv"):
        if row.get("qc_decision") == "TRY_ANOTHER_ROUTE" and row.get("sid"):
            sids.append(row["sid"])
    for row in read_csv(route4_root / "route_qc_decisions.csv"):
        if row.get("qc_decision") == "TRY_ANOTHER_ROUTE" and row.get("sid"):
            sids.append(row["sid"])
...
def main() -> int:
    ...
    parser.add_argument("--route4-root", type=Path, required=True)
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--mrtrix-threads", type=int, default=4)
    parser.add_argument("--variants", nargs="*", default=["endvox", "radial16", "radial24", "forward200"])
    args = parser.parse_args()

    run_root = args.route1_root.parent / f"{args.route1_root.name}_route5_deep_assignment"
    run_root.mkdir(parents=True, exist_ok=True)
    tag = run_root.name
    subjects = queue_subjects(args.route3_root, args.route4_root)
    ...
    write_json(
        ...
        {
            "mode": "route5_deep_endpoint_assignment_existing_tracks",
            "route1_root": str(args.route1_root),
            "route2_root": str(args.route2_root),
            "route3_root": str(args.route3_root),
            "route4_root": str(args.route4_root),
            "root": str(run_root),
            "tag": tag,
