...
"""Prepare or run corrected tractography for the archive-D0 56 lane."""
...
PRETRACT_WRAPPER_SOURCE = (
    EXP
    / "scripts/hcp/run_hcp379_archive_D0_pretract_recovery4_v3.py"
)
SUBSET_ROOT = (
    EXP
...
AUDIT_CSV = SUBSET_ROOT / "archive_D0_contingency_execution_52.csv"
AUDIT_SUMMARY = SUBSET_ROOT / "archive_D0_contingency_subset_summary.json"
...
EXECUTION_RELEASE_FILENAME = (
    "corrected_archive_D0_execution_52_release_manifest.json"
)
EXECUTION_RELEASE_RECORD_TYPE = (
    "diagnosis_blind_hcp379_corrected_archive_D0_execution_52_"
    "release_manifest"
)
COMPOSITE_RELEASE = (
    ROOT
...
EXPECTED_LANE_N = 56
EXPECTED_CALIBRATION_REUSE_N = 4
EXPECTED_EXECUTION_N = 52
...
D0_PRETRACT = load_module(
    PRETRACT_WRAPPER_SOURCE,
    "hcp379_archive_D0_pretract_recovery4_wrapper",
)
TRACT = load_module(
    SOURCE, "hcp379_archive_D0_tractography_recovery4_engine"
)


def specialize(root: Path) -> None:
    D0_PRETRACT.specialize(root)
    pretract = TRACT.PRETRACT
    pretract.AUDIT_CSV = AUDIT_CSV
    pretract.AUDIT_SUMMARY = AUDIT_SUMMARY
    pretract.OUTPUT_ROOT = root
    pretract.DEFAULT_HUMAN_QC = D0_PRETRACT.DEFAULT_HUMAN_QC
...
def write_wrapper_plan(
    ...
    contract = D0_PRETRACT.lane_contract()
    shared_path = (
        root
        / "manifests/"
        "tractography_recovery4_selected_recipe_plan.json"
    ...
    if (
        ...
        raise ValueError("archive D0 tractography pre-Phase plan differs")
    plan = {
        ...
        "records": {
            ...
            "shared_runner": TRACT.PRETRACT.file_record(SOURCE),
...
def verify_execution_manifest(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("archive D0 52-subject execution release differs")
    return manifest
...
def build_composite_release(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("archive D0 calibration reuse differs")
    ...
    for unit in sorted(contract["reuse_units"]):
        ...
        if (
            ...
            raise ValueError(f"{unit}:archive D0 calibration differs")
        records.append(
            {
                "unit": unit,
    ...
    expected = contract["execution_units"] | contract["reuse_units"]
    if (
        ...
        raise ValueError("archive D0 composite identity/density differs")
    ...
    manifest = {
        ...
        "records": {
            "execution_52": TRACT.PRETRACT.file_record(execution_path),
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("ARCHIVE_D0_TRACTOGRAPHY_RECOVERY4_SELF_TEST_PASS")
        return 0
    delegated = [
        str(SOURCE),
