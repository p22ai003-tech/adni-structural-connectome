#!/home/ec2-user/fsl/bin/python
"""Prepare or run Recovery4 pretract for the legacy/tensor density lane.

The exact lane contains 216 units: all 214 legacy-keep identities and both
tensor-repair identities.  Four are already Recovery4 Phase-B canaries, so
only 212 require new pretract execution.  This is deliberately one corrected
ACT route for the lane; it avoids retaining a density-selected mixture of
legacy and corrected processing.  The default is a non-imaging dry run.
"""

from __future__ import annotations

import argparse
import importlib.util
import json
import sys
from collections import Counter
from pathlib import Path
from typing import Any


EXP = Path("/home/ec2-user/exp")
HCP_ROOT = Path("/data/derivatives/hcp379_v2")
SOURCE = (
    EXP / "scripts/hcp/hcp379_scaleup_pretract_recovery4_v3.py"
)
AUDIT_ROOT = (
    EXP
    / "research_audit/outputs/"
    "hcp379_legacy_density_recovery_input_audit_v3"
)
AUDIT_CSV = AUDIT_ROOT / "legacy_density_recovery_input_audit.csv"
AUDIT_SUMMARY = (
    AUDIT_ROOT / "legacy_density_recovery_input_audit_summary.json"
)
DENSITY_TOPOLOGY = (
    EXP
    / "research_audit/outputs/hcp379_density_execution_topology_v3/"
    "hcp379_density_execution_subjects.csv"
)
OUTPUT_ROOT = HCP_ROOT / "corrected_legacy_tensor_recovery4"
DEFAULT_HUMAN_QC = (
    HCP_ROOT
    / "review_recovery4_corrected_overlay/"
    "human_visual_qc_recovery4_corrected_overlay.csv"
)
EXPECTED_AUDIT_N = 216
EXPECTED_CANARY_OVERLAP_N = 4
EXPECTED_EXECUTION_N = 212


def load_module(path: Path, name: str) -> Any:
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise ImportError(path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


PRETRACT = load_module(
    SOURCE, "hcp379_legacy_density_pretract_recovery4_engine"
)


def lane_contract() -> dict[str, Any]:
    audit = PRETRACT.load_json(AUDIT_SUMMARY)
    rows = PRETRACT.read_csv(AUDIT_CSV)
    topology_rows = [
        row
        for row in PRETRACT.read_csv(DENSITY_TOPOLOGY)
        if row["execution_lane"] == "corrected_legacy_tensor_216"
    ]
    units = {row["unit"] for row in rows}
    topology_units = {row["unit"] for row in topology_rows}
    recovery = PRETRACT.load_json(PRETRACT.RECOVERY4_MASTER)
    canaries = {
        str(row["unit"]) for row in recovery.get("units", [])
    }
    overlap = units & canaries
    groups = Counter(row["density_group"] for row in rows)
    if (
        audit.get("record_type")
        != "diagnosis_blind_hcp379_legacy_density_recovery_input_audit"
        or audit.get("status") != "PASS"
        or audit.get("diagnosis_or_outcomes_used") is not False
        or audit.get("audited_n") != EXPECTED_AUDIT_N
        or audit.get("ready_n") != EXPECTED_AUDIT_N
        or len(rows) != EXPECTED_AUDIT_N
        or len(units) != EXPECTED_AUDIT_N
        or topology_units != units
        or len(overlap) != EXPECTED_CANARY_OVERLAP_N
        or groups
        != {
            "D0_THRESHOLD_QUALIFIED": 2,
            "D1_NEAR_THRESHOLD": 69,
            "D2_MODERATE_LOW": 59,
            "D3_SEVERE_LOW": 68,
            "D4_CRITICAL_LOW": 18,
        }
        or any(
            row["route"] != "READY_FOR_CORRECTED_PRETRACT_FROM_EDDY"
            for row in rows
        )
    ):
        raise ValueError("legacy/tensor density lane contract differs")
    return {
        "units": units,
        "canary_overlap": overlap,
        "execution_units": units - canaries,
        "density_group_counts": dict(sorted(groups.items())),
    }


def specialize(root: Path) -> None:
    PRETRACT.AUDIT_CSV = AUDIT_CSV
    PRETRACT.AUDIT_SUMMARY = AUDIT_SUMMARY
    PRETRACT.OUTPUT_ROOT = root
    PRETRACT.DEFAULT_HUMAN_QC = DEFAULT_HUMAN_QC
    PRETRACT.EXPECTED_AUDIT_N = EXPECTED_AUDIT_N
    PRETRACT.EXPECTED_CANARY_OVERLAP_N = EXPECTED_CANARY_OVERLAP_N
    PRETRACT.EXPECTED_SCALEUP_N = EXPECTED_EXECUTION_N


def write_wrapper_plan(
    root: Path,
    *,
    execution_requested: bool,
    compact_reproducible_after_pass: bool,
) -> dict[str, Any]:
    contract = lane_contract()
    shared_path = root / "manifests/pretract_recovery4_plan.json"
    shared = PRETRACT.load_json(shared_path)
    expected_status = (
        "EXECUTION_GATE_PASS"
        if execution_requested
        else str(shared.get("status"))
    )
    if not execution_requested and expected_status not in {
        "PRE_HUMAN_DRY_RUN_PASS",
        "WAITING_FOR_530_HROI_POLICY_ADOPTION",
    }:
        raise ValueError("legacy/tensor shared HROI gate differs")
    if (
        shared.get("status") != expected_status
        or shared.get("execution_requested") is not execution_requested
        or shared.get("imaging_executed_by_plan") is not False
        or shared.get("diagnosis_labels_used") is not False
        or shared.get("target_n") != EXPECTED_EXECUTION_N
        or shared.get("selected_this_invocation_n")
        != EXPECTED_EXECUTION_N
        or shared.get("canary_overlap_excluded_n")
        != EXPECTED_CANARY_OVERLAP_N
        or {row.get("unit") for row in shared.get("units", [])}
        != contract["execution_units"]
    ):
        raise ValueError("legacy/tensor specialized pretract plan differs")
    plan = {
        "schema_version": "1.0.0",
        "record_type": (
            "diagnosis_blind_hcp379_legacy_tensor_density_"
            "pretract_recovery4_plan"
        ),
        "status": expected_status,
        "generated_utc": PRETRACT.utc_now(),
        "diagnosis_labels_used": False,
        "non_overwriting": True,
        "execution_requested": execution_requested,
        "imaging_executed_by_plan": False,
        "streaming_compaction_requested": (
            compact_reproducible_after_pass
        ),
        "lane_n": EXPECTED_AUDIT_N,
        "phase_b_primary_reuse_n": EXPECTED_CANARY_OVERLAP_N,
        "new_pretract_execution_n": EXPECTED_EXECUTION_N,
        "density_group_counts": contract["density_group_counts"],
        "output_root": str(root.resolve()),
        "canary_overlap_units": sorted(contract["canary_overlap"]),
        "execution_units": sorted(contract["execution_units"]),
        "records": {
            "input_audit": PRETRACT.file_record(AUDIT_CSV),
            "input_audit_summary": PRETRACT.file_record(AUDIT_SUMMARY),
            "density_execution_topology": PRETRACT.file_record(
                DENSITY_TOPOLOGY
            ),
            "shared_recovery4_plan": PRETRACT.file_record(shared_path),
            "shared_recovery4_runner": PRETRACT.file_record(SOURCE),
            "wrapper": PRETRACT.file_record(Path(__file__)),
        },
    }
    PRETRACT.atomic_json(
        root
        / "manifests/"
        "legacy_tensor_density_pretract_recovery4_plan.json",
        plan,
    )
    return plan


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--pre-human-dry-run", action="store_true")
    mode.add_argument("--execute", action="store_true")
    parser.add_argument(
        "--human-qc-manifest", type=Path, default=DEFAULT_HUMAN_QC
    )
    parser.add_argument("--root", type=Path, default=OUTPUT_ROOT)
    parser.add_argument("--workers", type=int, default=4)
    parser.add_argument("--nthreads", type=int, default=8)
    parser.add_argument(
        "--compact-reproducible-after-pass", action="store_true"
    )
    parser.add_argument("--self-test", action="store_true")
    args = parser.parse_args()

    contract = lane_contract()
    specialize(args.root)
    if args.self_test:
        if (
            len(contract["units"]) != EXPECTED_AUDIT_N
            or len(contract["canary_overlap"])
            != EXPECTED_CANARY_OVERLAP_N
            or len(contract["execution_units"]) != EXPECTED_EXECUTION_N
        ):
            raise AssertionError(contract)
        print("LEGACY_DENSITY_PRETRACT_RECOVERY4_SELF_TEST_PASS")
        return 0

    delegated = [
        str(SOURCE),
        "--execute" if args.execute else "--pre-human-dry-run",
        "--human-qc-manifest",
        str(args.human_qc_manifest),
        "--root",
        str(args.root),
        "--workers",
        str(args.workers),
        "--nthreads",
        str(args.nthreads),
    ]
    if args.compact_reproducible_after_pass:
        delegated.append("--compact-reproducible-after-pass")
    previous = sys.argv
    try:
        sys.argv = delegated
        returncode = int(PRETRACT.main())
    finally:
        sys.argv = previous
    if returncode:
        return returncode
    plan = write_wrapper_plan(
        args.root,
        execution_requested=bool(args.execute),
        compact_reproducible_after_pass=(
            args.compact_reproducible_after_pass
        ),
    )
    print(json.dumps(plan, indent=2, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
