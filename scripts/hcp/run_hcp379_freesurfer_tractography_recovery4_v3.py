...
"""Prepare or run selected-count corrected tractography for the FS unit."""
...
PRETRACT_WRAPPER_SOURCE = (
    EXP
    / "scripts/hcp/run_hcp379_freesurfer_pretract_recovery4_v3.py"
)
AUDIT_ROOT = (
    EXP
...
RELEASE_FILENAME = "corrected_freesurfer_1_release_manifest.json"
RELEASE_RECORD_TYPE = (
    "diagnosis_blind_hcp379_corrected_freesurfer_1_release_manifest"
)
EXPECTED_N = 1
...
FS_PRETRACT = load_module(
    PRETRACT_WRAPPER_SOURCE,
    "hcp379_freesurfer_pretract_recovery4_wrapper",
)
TRACT = load_module(
    SOURCE, "hcp379_freesurfer_tractography_recovery4_engine"
)


def specialize(root: Path) -> None:
    FS_PRETRACT.specialize(root)
    pretract = TRACT.PRETRACT
    pretract.AUDIT_CSV = AUDIT_CSV
    pretract.AUDIT_SUMMARY = AUDIT_SUMMARY
    pretract.OUTPUT_ROOT = root
    pretract.DEFAULT_HUMAN_QC = FS_PRETRACT.DEFAULT_HUMAN_QC
...
def write_wrapper_plan(
    ...
    units = FS_PRETRACT.lane_contract()
    shared_path = (
        root
        / "manifests/"
        "tractography_recovery4_selected_recipe_plan.json"
    ...
    if (
        ...
        raise ValueError("FreeSurfer tractography pre-Phase plan differs")
    plan = {
        "schema_version": "1.0.0",
        "record_type": (
            "diagnosis_blind_hcp379_freesurfer_tractography_"
            "recovery4_plan"
        ),
        ...
        "lane_n": EXPECTED_N,
        "new_tractography_execution_n": EXPECTED_N,
        "minimum_density_inclusive": 0.60,
        "required_lane_density_pass_n": EXPECTED_N,
...
def verify_release(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or str(units[0].get("unit")) != UNIT
        or float(manifest.get("minimum_observed_edge_density", -1.0))
        < 0.60
        or float(units[0].get("edge_density", -1.0)) < 0.60
    ):
        raise ValueError("FreeSurfer corrected release differs")
    return manifest
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("FREESURFER_TRACTOGRAPHY_RECOVERY4_SELF_TEST_PASS")
        return 0
    delegated = [
        str(SOURCE),
