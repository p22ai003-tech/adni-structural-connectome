...
"""Prepare or run proactive Recovery4 pretract for 52 archive-D0 units."""
...
OUTPUT_ROOT = HCP_ROOT / "corrected_archive_D0_recovery4"
DEFAULT_HUMAN_QC = (
    HCP_ROOT
...
PRETRACT = load_module(
    SOURCE, "hcp379_archive_D0_pretract_recovery4_engine"
)


def lane_contract() -> dict[str, set[str]]:
    ...
    if (
        ...
        != "diagnosis_blind_hcp379_archive_D0_contingency_input_subset"
        ...
        or any(
            ...
            or float(row["current_density"]) < 0.60
            or row["route"]
            not in {
        ...
        raise ValueError("archive D0 contingency lane contract differs")
...
def write_wrapper_plan(
    ...
    *,
    ...
    if not execution_requested and expected_status not in {
        ...
        raise ValueError("archive-D0 shared HROI gate differs")
    if (
        ...
        raise ValueError("archive D0 specialized pretract plan differs")
    plan = {
        ...
        "proactive_before_route_decision": True,
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("ARCHIVE_D0_PRETRACT_RECOVERY4_SELF_TEST_PASS")
        return 0
    delegated = [
        str(SOURCE),
