#!/usr/bin/env python3
"""Targeted AD post/connectome recovery for EC2-local derivatives.

This runner is intentionally narrow: it selects AD image-series IDs that already
have final tracks and complete DTI scalar maps but are missing complete Step 7
post/connectome outputs. It then runs only Step 7 ``post`` for that list.
"""
...
def build_targets(deriv_root: Path, group_map: dict[str, str]) -> tuple[list[str], dict[str, set[str]]]:
    ...
    complete_post = {
        sid
        for sid in tracks | dti | connectome_sids(conn_dir, "ALL")
        if post_complete(conn_dir, sid)
    }
    targets = sorted(
        sid
        for sid in tracks & dti
        if group_map.get(subject_id(sid)) == "AD" and sid not in complete_post
    )
    return targets, {
        "tracks": tracks,
        "dti": dti,
...
def copy_if_exists(src: Path, dst_root: Path, deriv_root: Path) -> None:
    if not src.exists():
        return
    rel = src.relative_to(deriv_root) if src.is_relative_to(deriv_root) else Path(src.name)
    dst = dst_root / rel
...
def backup_existing_target_outputs(deriv_root: Path, run_root: Path, sids: list[str]) -> None:
    ...
    for sid in sids:
        ...
        conn_dir = deriv_root / "connectomes"
        for path in conn_dir.glob(f"SC_AAL_{sid}_*.csv"):
            copy_if_exists(path, backup_root, deriv_root)


def write_target_inventory(run_root: Path, deriv_root: Path, sids: list[str], sets: dict[str, set[str]]) -> None:
    run_root.mkdir(parents=True, exist_ok=True)
    (run_root / "targets.txt").write_text("\n".join(sids) + ("\n" if sids else ""), encoding="ascii")
    ...
    counts = {
        ...
        "ad_tracks": sum(1 for sid in sets["tracks"] if "_S_" in sid),
        "deriv_root": str(deriv_root),
        "required_post_suffixes": list(REQUIRED_POST_SUFFIXES),
    }
    (run_root / "run_metadata.json").write_text(json.dumps(counts, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    targets, sets = build_targets(paths.deriv_root, group_map)
    if args.limit and args.limit > 0:
        targets = targets[: args.limit]
    write_target_inventory(args.run_root, paths.deriv_root, targets, sets)

    print(
        f"AD connectome recovery | {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}",
        flush=True,
    )
    print(f"derivatives: {paths.deriv_root}", flush=True)
    print(f"targets: {len(targets)} | batch_size={args.batch_size} | execute={args.execute}", flush=True)
    ...
    print("Step 7 post recovery finished.", flush=True)
