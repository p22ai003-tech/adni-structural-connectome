...
"""Build the proactive corrected-recovery subset for 56 archive D0 units."""
...
FULL_CSV = OUTPUT_ROOT / "archive_D0_contingency_56.csv"
EXECUTION_CSV = OUTPUT_ROOT / "archive_D0_contingency_execution_52.csv"
SUMMARY = OUTPUT_ROOT / "archive_D0_contingency_subset_summary.json"
...
def build() -> dict[str, Any]:
    ...
    D0_topology = {
        ...
        if row["execution_lane"] == "conditional_archive_D0_56"
    }
    if len(D0_topology) != 56 or not set(D0_topology).issubset(audit):
        raise ValueError("archive D0 topology is not exact 56")
    ...
    reuse_units = set(D0_topology) & calibration_units
    execution_units = set(D0_topology) - reuse_units
    if len(reuse_units) != 4 or len(execution_units) != 52:
        raise ValueError("archive D0 calibration/execution split differs")

    full_rows = []
    execution_rows = []
    for unit in sorted(D0_topology):
        row = dict(audit[unit])
        row.update(
            {
                "density_group": D0_topology[unit]["density_group"],
                "current_density": D0_topology[unit]["current_density"],
                "archive_calibration_primary_reuse": unit in reuse_units,
                "proactive_pretract_contingency": unit in execution_units,
    ...
    if groups != {"D0_THRESHOLD_QUALIFIED": 56}:
        raise ValueError(f"archive D0 groups differ: {groups}")
    if any(
        float(row["current_density"]) < 0.60
        or row["route"]
        not in {
        ...
        raise ValueError("archive D0 subset contains a nonready input")
    ...
    summary = {
        ...
        "record_type": (
            "diagnosis_blind_hcp379_archive_D0_"
            "contingency_input_subset"
        ),
        "status": "PASS",
        ...
        "audited_n": 52,
        "ready_n": 52,
        "archive_calibration_primary_reuse_n": 4,
        "new_execution_n": 52,
        ...
        "pretract_may_run_proactively_before_route_decision": True,
        "tractography_requires_phase_b_selected_uniform_count": True,
        "corrected_release_is_required_if_archive_concordance_fails": True,
        "records": {
