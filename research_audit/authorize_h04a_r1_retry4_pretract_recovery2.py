#!/usr/bin/env python3
"""Release the exact retry4 pretract Recovery2 package after user approval."""
...
DECISION_CANDIDATE = PACKAGE / "recovery2_decision_proposed.json"
DECISION_APPROVED = PACKAGE / "recovery2_decision_approved.json"
EXECUTION_BINDING = PACKAGE / "recovery2_execution_binding.json"
...
def main() -> int:
    ...
    if any(path.exists() for path in outputs):
        raise FileExistsError("Recovery2 approval release already exists or is incomplete")
    ...
    if (
        ...
        raise ValueError("Recovery2 decision candidate differs")
    for record in candidate.get("source_files", {}).values():
        if file_record(Path(str(record["path"]))) != record:
            raise ValueError(f"Recovery2 source drifted: {record.get('path')}")
    ...
    binding = {
        ...
        "record_type": "retry4_pretract_recovery2_execution_binding",
        "status": "LOCKED",
        "run_root": candidate["run_root"],
        "units": candidate["units"],
        "recovery2_decision": file_record(DECISION_APPROVED),
        "prior_recovery1_completion": candidate["prior_recovery1_completion"],
    ...
    release = {
        "schema_version": "1.0.0",
        "record_type": "retry4_pretract_recovery2_approval_release_validation",
        "status": "PASS",
        "approved_utc": approved_utc,
        "user_response": REQUIRED_USER_RESPONSE,
        "package_validation": file_record(VALIDATION),
        "recovery2_decision": file_record(DECISION_APPROVED),
        "recovery2_execution_binding": file_record(EXECUTION_BINDING),
